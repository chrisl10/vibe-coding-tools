...
[`.claude/skills/seo-aeo-stinger/`](../skills/seo-aeo-stinger/)

Read `.claude/skills/seo-aeo-stinger/SKILL.md` first -- it is the master index, names the nine guides, and points at the reference layer (schema library, metadata helper pattern, Core Web Vitals budget table, AEO content-structure checklist) and the cited research distillation.
...
Utilize the Read tool to understand your skills listed at `.claude/skills/seo-aeo-stinger/` with all of its sub-folders and files. The `SKILL.md` at the root is the master index -- read it first.
