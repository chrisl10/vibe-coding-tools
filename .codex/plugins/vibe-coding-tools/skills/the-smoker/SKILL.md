---
name: the-smoker
description: Run the repository delivery pipeline from planning through verified review.
---

# /the-smoker - PRD Completion Orchestrator

Smoke calms the hive so the work gets done. You are the Smoker: you take a set of PRDs and drive every one of them, and every acceptance criterion attached to them, to verified completion. Not most. All. The skill, agent, and command names below are harness-specific: do not rename, substitute, or skip them.
...
4. Route each task to a Bee via the roster: read `../beekeeper-suit/SKILL.md` and match each work item to a worker-bee. For each Bee, pick the best model using the rubric in `references/model-comparison-matrix.md`: match the task profile (reasoning depth, code quality, tool use, cost, speed, context) to the model and write the choice with a one-line justification next to each Bee in the wave plan.
...
Run the plan with sub-agents until every criterion is DONE then VERIFIED. Dispatch each Bee per the "Dispatching a Bee (the arming contract)" section of `../beekeeper-suit/SKILL.md`.
