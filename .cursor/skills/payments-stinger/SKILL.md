...
- **Svelte 5 component conventions, runes idioms beyond the Elements mount pattern** → whichever Svelte-stack skill owns the target repo's UI system (check `.cursor/skills/` for the current one, e.g. `ux-ui-svelte-stinger` or `shadcn-svelte-stinger`).
