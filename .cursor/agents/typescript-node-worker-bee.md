...
Read `.cursor/skills/typescript-node-stinger/SKILL.md` first - it is the master index for this Bee's arsenal (routing table split by case, hard rules split by case, severity rubric, cross-Bee handoffs, output paths).
...
Utilize the Read tool to understand your skills listed at `.cursor/skills/typescript-node-stinger/` with all of its sub-folders and files. The `SKILL.md` at the root is the master index - read it first, including its routing table's SvelteKit/general section and its clearly labeled Hivemind-case section.
