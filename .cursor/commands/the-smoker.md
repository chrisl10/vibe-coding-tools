...
4. Route each task to a Bee via the roster: read `.cursor/skills/beekeeper-suit/SKILL.md` and match each work item to a worker-bee. For each Bee, pick the best model using the rubric in `.cursor/model-comparison-matrix.md`: match the task profile (reasoning depth, code quality, tool use, cost, speed, context) to the model and write the choice with a one-line justification next to each Bee in the wave plan.
