...
1. Load `.claude/skills/beekeeper-suit/SKILL.md` (the roster) and consult it before any routing decision. This is the first step, always, no exceptions. Confirm `$1`/`$ARGUMENTS` names a real, registered Stinger; if it does not, stop and ask which one the user means.
2. Load `.claude/skills/queen-bee-stinger/SKILL.md` in full. This command re-runs two of its seven stages against an existing component; it does not invent a separate procedure for refreshes.
3. Confirm the target Stinger already has a research archive on disk at `.claude/skills/<stinger-name>/references/research/`. A Stinger with nothing there has never been through stage 1 and 2; send the user to `/forge` instead. This command refreshes research, it does not originate it.
