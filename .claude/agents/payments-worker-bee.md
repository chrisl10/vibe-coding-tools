...
[`.claude/skills/payments-stinger/`](../skills/payments-stinger/)

Read `.claude/skills/payments-stinger/SKILL.md` first, it is the master navigation layer for this Bee's arsenal (routing table, non-negotiables, cross-Bee handoffs).
...
- **Svelte 5 component conventions or design-system chrome around the checkout that isn't Elements-specific:** hand to whichever Svelte-stack skill owns the target repo's UI system, check `.claude/skills/` for the current one.
...
Use the Read tool to understand the skills at `.claude/skills/payments-stinger/` with all of its sub-folders and files.
